...
library transaction {
    ...
    function publish(
        ...
        string memory seqNo,
        string memory publishId,
        uint256 price,
        bytes memory metaDataIdEncSeller,
        bytes32[] memory proofDataIds,
        string memory descDataId,
        bool supportVerify
    ) external {
    ...
    function reEncrypt(
        ...
        string memory seqNo,
        uint256 txId,
        bytes memory encryptedMetaDataIdWithBuyer,
        bytes memory encryptedMetaDataIdWithArbitrators
    ) external {
        common.TransactionItem storage txItem = ds.txData.map[txId];
        require(txItem.used, "Transaction does not exist");
    ...
    function deserializeAndSave(common.TransactionItem storage txItem, bytes memory encIds, uint8 num) internal {
    ...
    function arbitrateResult(common.DataSet storage ds, string memory seqNo, uint256 txId, ERC20 token) internal {
        uint8 truth;
        for (uint8 i = 0;i < ds.conf.arbitratorNum;i++) {
    ...
    function getArbitrators(common.DataSet storage ds, uint256 txId) internal view returns (address[] memory) {
        common.TransactionItem memory txItem = ds.txData.map[txId];
        require(msg.sender == txItem.seller, "Invalid caller");
